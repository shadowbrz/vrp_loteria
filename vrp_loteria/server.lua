...
function vRPlt.buyTickets(amount)
  ...
  if ((#tickets[user_id])+amount) <= 5 then -- verifica se chegou ou limite de tickets (tickets atuais + quantidade comprada)
    if vRP.tryPayment(tonumber(user_id),500*amount) then -- verifica o pagamento 100*quantidade comprada
	  ...
	  TriggerClientEvent("chatMessage", -1, "[Loteria]", {255, 255, 0}, "Você comprou:" .. amount .. " ticket(s)")
	  ...
	  TriggerClientEvent("chatMessage", -1, "[Loteria]", {255, 255, 0}, "Você não tem dinheiro suficiente!")
    end
  else
    TriggerClientEvent("chatMessage", -1, "[Loteria]", {255, 255, 0}, "Você já alcançou o número máximo de tickets")
  end
end
...
function tick_lotto()
    ...
    for user_id,u_tickets in pairs(tickets) do -- para user_id e u_tickets (tickets do usuario)
       for i,ticket in pairs(u_tickets) do -- para ticket de u_tickets (tickets do usuario)
         if no_winner and ticket == winner then -- encontra ganhador se ninguem ganhou
           vRP.giveMoney(tonumber(user_id),money) -- da o dinheiro ao ganhador
  ...
  TriggerClientEvent("chatMessage", -1, "[Loteria]", {255, 255, 0}, "Essa rodada da lotto se encerra em " .. math.ceil(30-((os.time() - old_time )/60)) .." minutos! Nao perca essa!")
